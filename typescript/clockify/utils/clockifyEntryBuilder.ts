        ...
        return new Promise(async (res, rej) => {
            try {
                switch (this.day.getDay()) {
                    case 0:
                        await this.clockifyClient.sendRequest({ startTime:start, endTime:start, projectId:data.sunProjectId });
                        res("request sent for Sunday");
                        break;
                    case 6:
                        await this.clockifyClient.sendRequest({ startTime:start, endTime:start, projectId:data.satProjectId });
